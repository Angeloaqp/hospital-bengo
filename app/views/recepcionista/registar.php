...
<head>
    ...
    <style>
        ...
        .tactile-input {
            background-color: var(--cor-surface-container-low); /* surface-container-low */
            border: none;
            transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
        }
        .tactile-input:hover {
            background-color: var(--cor-surface-container); /* surface-container */
        }
        .tactile-input:focus {
            background-color: var(--cor-surface-container-lowest);
        ...
        .btn-gradient {
            background: linear-gradient(135deg, var(--cor-toast-bg) 0%, var(--cor-primary-container) 100%);
        }
    </style>
</head>
...
<div class="ml-0 lg:ml-[17rem] lg:mr-6 px-4 sm:px-6 lg:px-0 mt-28 pb-24 lg:pb-8 flex justify-center min-h-screen">
    <main class="w-full relative mt-4">
        ...
        <div class="mb-4 transition-all duration-500 fade-in" id="page-header">
            ...
            <p class="font-body text-on-surface-variant mt-2 text-sm">Preencha os detalhes abaixo para registar um novo paciente no sistema HGB.</p>
        </div>
        ...
        <form method="POST" action="<?= BASE_URL ?>app/controllers/pacientes_api.php" id="form-registo" class="bg-white rounded-[32px] floating-card border border-white/50 overflow-hidden relative z-10 transition-all duration-500 fade-in-delay-1">
            ...
            <!-- ERROS -->
            <?php if (!empty($erros)): ?>
                <div class="bg-[var(--cor-error-banner-bg)] text-[var(--cor-error-banner-text)] px-6 py-5 mx-8 mt-8 rounded-[1.5rem] text-sm font-bold shadow-sm flex gap-3 items-start">
            ...
            <div class="p-8 md:p-12 flex flex-col gap-12" id="section-form-content">
                ...
                <section>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <div class="bg-surface-container-low p-3.5 flex items-center input-focus-ring rounded-2xl bg-surface-container-low/50">
                ...
                <section>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                        ...
                        <div class="flex flex-col gap-2 md:col-span-2">
                            ...
                            <div class="bg-surface-container-low p-3.5 flex items-start input-focus-ring rounded-2xl bg-surface-container-low/50">
                ...
                <div class="pt-6 border-t border-surface-container flex items-center justify-end gap-4">
                    <button type="button" onclick="window.history.back()" class="px-6 py-3 font-headline font-bold text-sm text-on-surface-variant hover:bg-surface-container-low transition-colors duration-300 ease-in-out rounded-2xl">
                        Cancelar
                    </button>
        ...
        <div id="section-sucesso" class="min-h-[70vh] flex flex-col items-center justify-center text-center opacity-0 pointer-events-none absolute inset-0 transition-all duration-700 translate-y-8 z-0 hidden w-full">
            <div class="mb-12 flex flex-col items-center">
                ...
                <p class="font-body text-on-surface-variant text-lg max-w-xl">O registo do paciente foi guardado na base de dados com sucesso. Escolha o próximo passo lógico.</p>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-4xl mb-12">
                ...
                <a href="#" id="link-mesmo-dia" class="bg-white p-10 rounded-[2.5rem] floating-card border border-white/50 text-left flex flex-col relative group cursor-pointer hover:-translate-y-1 hover:shadow-[0_12px_30px_-4px_rgba(0,0,0,0.1)] transition-all duration-300">
                    <div class="w-14 h-14 bg-surface-container-low rounded-2xl flex items-center justify-center mb-6">
                    ...
                    <p class="font-body text-on-surface-variant text-sm pr-12">Encaminhar o paciente agora mesmo para a triagem ou urgência.</p>
                ...
                <a href="#" id="link-marcacao" class="bg-white p-10 rounded-[2.5rem] floating-card border border-white/50 text-left flex flex-col relative group cursor-pointer hover:-translate-y-1 hover:shadow-[0_12px_30px_-4px_rgba(0,0,0,0.1)] transition-all duration-300">
                    ...
                    <p class="font-body text-on-surface-variant text-sm pr-12">Agendar consulta médica para uma data e horário futuro.</p>
                    <div class="absolute bottom-8 right-8 w-12 h-12 bg-primary text-white rounded-full flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform duration-300">
                        <span class="material-symbols-outlined">arrow_forward</span>
                    </div>
                </a>
            ...
            <div class="flex items-center justify-center gap-4 mb-12">
                <button type="button" onclick="editarPaciente()" class="flex items-center gap-2 px-6 py-3 bg-surface-container-low text-on-surface-variant font-headline font-bold text-sm rounded-2xl hover:bg-surface-container-high transition-colors">
                    <span class="material-symbols-outlined text-[20px]">edit</span>
                    Editar Dados
                </button>
                <button type="button" onclick="window.location.reload()" class="flex items-center gap-2 px-6 py-3 bg-surface-container-low text-on-surface-variant font-headline font-bold text-sm rounded-2xl hover:bg-surface-container-high transition-colors">
