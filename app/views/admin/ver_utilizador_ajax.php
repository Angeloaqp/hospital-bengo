...
<div class="flex flex-col gap-6 fade-in">
    ...
    <div class="flex items-center gap-4">
        <div class="w-16 h-16 rounded-full overflow-hidden bg-black text-white flex items-center justify-center font-bold text-2xl shrink-0 ring-2 ring-surface-container-low">
        ...
        <div>
            <h3 class="text-xl font-headline font-black text-black leading-tight"><?= htmlspecialchars($dados['nome']) ?></h3>
    ...
    <div class="grid grid-cols-2 gap-3">
        <div class="bg-surface-container-low p-3 rounded-2xl border border-black/5">
            ...
            <span class="font-bold text-black text-sm">#<?= $dados['id'] ?></span>
        </div>
        <div class="bg-surface-container-low p-3 rounded-2xl border border-black/5">
            <span class="block text-[9px] uppercase font-black tracking-widest text-on-surface-variant mb-1">Username</span>
            <span class="font-bold text-black text-sm">@<?= htmlspecialchars($dados['nome_utilizador']) ?></span>
        </div>
        <div class="col-span-2 bg-surface-container-low p-3 rounded-2xl border border-black/5">
            ...
            <span class="font-bold text-black text-sm"><?= htmlspecialchars($dados['telefone'] ?: 'Não definido') ?></span>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 gap-3">
        <div class="bg-white p-4 rounded-2xl border border-surface-container shadow-sm flex flex-col items-center justify-center text-center">
            <span class="text-3xl font-black tactile-mono text-black"><?= $estatisticas['hoje'] ?></span>
            <span class="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest mt-1">Hoje</span>
        </div>
        <div class="bg-white p-4 rounded-2xl border border-surface-container shadow-sm flex flex-col items-center justify-center text-center">
            <span class="text-3xl font-black tactile-mono text-black"><?= $estatisticas['semana'] ?></span>
            <span class="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest mt-1">Semana</span>
        </div>
    </div>

    <div class="mt-4 flex flex-col gap-2">
        <a href="ver_utilizador.php?id=<?= $dados['id'] ?>" class="w-full bg-black text-white rounded-xl py-3 font-bold text-sm text-center hover:scale-[1.02] transition-transform shadow-md">
            Ver Perfil Completo
        </a>
        <a href="editar_utilizador.php?id=<?= $dados['id'] ?>" class="w-full bg-surface-container-low text-black border border-black/5 rounded-xl py-3 font-bold text-sm text-center hover:bg-surface-container transition-colors">
            Editar Conta
        </a>
    </div>
</div>
