...
# Definições de conteúdo das abas e layout
PHP_HEADER = """<?php
// ================================================
...
<body class="text-on-surface bg-[#f3f4f6] antialiased selection:bg-primary-container selection:text-on-primary-container">
...
HTML_MAIN_TOP = """
<main class="ml-56 pl-6 pt-24 h-screen overflow-y-auto custom-scrollbar">
<div class="p-8 w-full  mx-auto min-h-full pb-24 space-y-8">
                ...
                <form method="POST" action="<?= BASE_URL ?>app/controllers/admin_config.php" class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-2">
                            ...
                            <input type="date" name="data_disponibilidade" required min="<?= date('Y-m-d') ?>" class="input-recessed w-full rounded-2xl px-5 py-4 text-sm font-medium focus:ring-2 focus:ring-primary/20">
                        </div>
                        <div class="space-y-2">
            ...
            <div class="bg-white rounded-[2.5rem] p-8 shadow-[0_12px_40px_rgb(0,0,0,0.06)] border border-surface-container-high panel-enter">
                ...
                <form method="POST" action="<?= BASE_URL ?>app/controllers/admin_config.php" class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-2">
                            <label class="text-xs font-extrabold text-on-surface tracking-wider uppercase pl-1">Data</label>
                            <input type="date" name="data_bloqueio" required min="<?= date('Y-m-d') ?>" class="input-recessed w-full rounded-2xl px-4 py-4 text-sm font-medium focus:ring-2 focus:ring-error/20">
                        </div>
                        <div class="space-y-2">
                            <label class="text-xs font-extrabold text-on-surface tracking-wider uppercase pl-1">Turno</label>
...
with open(r"c:\xampp\htdocs\hospital-bengo\app\views\admin\configuracao.php", "w", encoding="utf-8") as f:
    f.write(PHP_HEADER)
    f.write(HTML_HEAD)
    f.write(HTML_MAIN_TOP)
    f.write(HTML_TAB_CONSULTORIOS)
    f.write(HTML_TAB_ESPECIALIDADES)
    f.write(HTML_TAB_TIPOS)
    f.write(HTML_TAB_DISPONIBILIDADE)
    f.write(HTML_TAB_VINCULOS)
    f.write(HTML_TAB_BLOQUEIOS)
    f.write(HTML_FOOTER)
